...
static void APIENTRY gl_debug_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* user){
    if(severity == GL_DEBUG_SEVERITY_LOW || severity == GL_DEBUG_SEVERITY_MEDIUM || severity == GL_DEBUG_SEVERITY_HIGH){
        SP_ASSERT(0, "OpenGL Error %s", message);
    }
    else{
        SP_TRACE((char*)message);
    }
}
...
bool gl_init(BumpAllocator* transientStorage){
    ...
    if(!vertShader || !fragShader){
        SP_ASSERT(false, "Failed to load shaders");
        return false;
    }
...
if(!success){
    ...
    SP_ASSERT(false, "Failed to compile Vertex Shaders %s", shaderLog);
}
    }
    {
int success;
...
if(!success){
    ...
    SP_ASSERT(false, "Failed to compile Fragment Shaders %s", shaderLog);
}
    }
    ...
    //must be done otherwise OpenGL will not draw anything.
    GLuint VAO;
    ...
    // Texture Loading using STBI
    {
        ...
        if(!data){
            SP_ASSERT(false, "failed to load texture: %s", stbi_failure_reason());
            return false;
        }
    ...
    // Of course, you still need to correcly set the image file source when using glTexImage2D()
